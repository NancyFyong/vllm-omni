...
from .sampling import SAMPLING_EPS, ras_sample_semantic
...
class Audio8TTSSlowARForConditionalGeneration(nn.Module):
    ...
    def sample(self, logits: torch.Tensor, sampling_metadata: Any) -> SamplerOutput | None:
        ...
        if sampling_metadata.all_greedy or temperature is None:
            ...
            local_ids = ras_sample_semantic(
                compact,
                self._recent_local_ids(sampling_metadata.output_token_ids, num_reqs, device),
                ...
                # One shared generator per batch: per-request generators would
                # force a Python loop over requests.
    ...
    def talker_mtp(
        ...
        **kwargs: Any,
        ...
        audio_codes = self.fast_ar(
            ...
            top_p=float(kwargs.get("top_p") or DEFAULT_TOP_P),
            generator=generator,
        )
