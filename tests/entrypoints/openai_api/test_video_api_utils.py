...
def test_prepared_automatic_fallback_preserves_legacy_quantization(monkeypatch, video):
    ...
    # uint8 frames reach the muxer untouched; the /255 then *255 round trip the
    # float path performs is an exact identity for them.
    if stacked.dtype == np.uint8:
        reference = stacked
    else:
        reference = np.rint(np.clip(stacked, 0.0, 1.0) * 255.0).astype(np.uint8)
