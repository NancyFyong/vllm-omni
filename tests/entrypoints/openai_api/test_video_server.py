...
def test_raw_and_base64_encoders_receive_no_policy_config(mocker: MockerFixture):
    ...
    # Both endpoints encode through _encode_video_bytes now; the base64 one
    # b64-encodes the returned bytes itself instead of calling a second encoder.
    assert raw_encoder.call_count == 2
    for call in raw_encoder.call_args_list:
        assert "encoding_config" not in call.kwargs
