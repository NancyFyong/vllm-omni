...
def test_text_to_audio_concurrent_002(omni_runner: OmniRunner, run_level: str) -> None:
    ...
    Per-request codec state is keyed by request id; a shared buffer shows up
    here as truncation or crosstalk, and only here.
    """
    prompt = _text_only_prompt()
    request_config = {
        "input": TEXT,
        "prompt_token_ids": [prompt["prompt_token_ids"]] * 4,
        "additional_information": [prompt["additional_information"]] * 4,
        "response_format": "wav",
        "expected_sample_rate": SAMPLE_RATE,
        **_duration_bounds(run_level),
    }
